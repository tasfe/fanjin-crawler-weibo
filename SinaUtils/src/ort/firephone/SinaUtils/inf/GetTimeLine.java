...
public class GetTimeLine extends EatAppleMuti {
	...
	public boolean eat(Object arg0) throws Exception {
		...
		try {
			FSstatusU s_status_u = new FSstatusU();
			s_status_u.setUser_id(user_id);
			s_status_u.setCount(userTimeLineCount);
			s_status_u.insertIntoDB();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return proc_status;
	...
	public void processAllApple() {
		try {
			Vector<Long> all_user_ids = Fuser.getAllIdsNotInFSstatusU(ONCE_MAX);
			while (all_user_ids.size() > 0) {
				for (int i = 0; i < all_user_ids.size(); i++) {
					long user_id = all_user_ids.get(i);
				...
				all_user_ids = Fuser.getAllIdsNotInFSstatusU(ONCE_MAX);
			}
